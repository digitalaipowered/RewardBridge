...
function RewardBridgeApp() {
  ...
  const savePaypalEmail = async () => {
    ...
    setLoading(false);
    if (error) { setNotice(error.message); return; }
    setPaypalEmail(''); setNotice('Payout destination submitted for verification.'); await loadData();
  };
