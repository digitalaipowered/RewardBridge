...
function RewardBridgeApp() {
  ...
  const heldBalance = useMemo(() => ledger.filter(item => item.balance_state === 'held' && item.account_scope === 'publisher').reduce((sum, item) => sum + Number(item.amount_usd), 0), [ledger]);
